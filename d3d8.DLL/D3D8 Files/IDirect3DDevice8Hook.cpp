...
{
	if (g_CustomDevice)
		g_CustomDevice->LostDevice();

	HRESULT hr = pD3DDevice->Reset(pPresentationParameters);

	if (SUCCEEDED(hr) && g_CustomDevice)
		g_CustomDevice->ResetDevice();
...
{
	if (g_Computron && g_Computron->initialized)
		g_Computron->IncrementSurfaceCount(PrimitiveCount);
